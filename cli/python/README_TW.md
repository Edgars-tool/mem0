...
這是 [mem0](https://mem0.ai) 的官方命令列介面 — AI 代理的記憶層。Python 實作。
...
> **注意：** 在 macOS 上使用 Homebrew Python 時，在虛擬環境外使用 `pip install` 會因為 `externally-managed-environment` 錯誤而失敗（[PEP 668](https://peps.python.org/pep-0668/)）。請改用 `pipx`，或在虛擬環境內安裝。
...
2. 建立 GitHub Release 並加上標籤 `cli-v<version>`（例如 `cli-v0.2.1`）

對於預發布版本，請使用類似 `0.2.1b1` 的測試版本，並勾選 **pre-release** 核取方塊。
